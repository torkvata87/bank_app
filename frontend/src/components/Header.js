...
/**
 * Класс Header отвечает за создание заголовка страницы.
 */
export class Header {
  /**
   * Рендерит заголовок страницы.
   */
  renderHeader(navigation, pageName) {
    document.body.innerHTML = '';
    this.header = el(
      'header.header',
      this.renderHeaderContainer(navigation, pageName),
    );
    return this.header;
  }

  /**
   * Рендерит контейнер заголовка с логотипом и навигацией.
   */
  renderHeaderContainer(navigation, pageName) {
    let headerContainer; // Переменная для хранения контейнера заголовка.
  ...
  /**
   * Создает навигационное меню на основе переданных ссылок.
   */
  createNavigation(pageName) {
    const navigationList = el('ul.nav__list.list-reset', [
      this.createNavLink('Банкоматы', '/atm', pageName),
      this.createNavLink('Счета', '/accounts-all', pageName),
      this.createNavLink('Валюта', '/currency', pageName),
      this.createNavLink('Выйти', '/login', pageName),
    ]);
    return el('.header__nav', navigationList);
  }
}
